...
`npm run dev` runs against the gateway with local fallbacks (e.g. local-FS
prefs) — enough to build UI. To run a **BC 2.0 tenant** (`hosting: gcp` in
`broadchurch.yaml`) against its **real per-tenant data planes** (Firestore,
BigQuery, Cloud SQL, the agent), run `npm run bridge` first — see
[local dev against real data planes](.agents/skills/aether/local-dev-bc2.md)
(or the `/bridge` command in Cursor / Claude Code).
...
- **App**: push to `main` deploys it — via the **Deploy Aether UI** GitHub Actions workflow (`hosting: gcp`) or Vercel (`hosting: vercel`), per `broadchurch.yaml`
