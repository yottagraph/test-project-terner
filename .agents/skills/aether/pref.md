...
### Mirroring prefs into a side store — watch `{ deep: true }`

A common BC 2.0 pattern: a pref is the source of truth in the UI, but a
**compute job** needs it too — and jobs don't read Firestore (see
[`compute.md`](compute.md) / [`storage.md`](storage.md)). The handoff is to
**mirror the pref into Cloud SQL** (the UI writes a canonical row; the job
reads it). When you wire that mirror with a `watch`, watch the **reactive
proxy itself with `{ deep: true }`** — not a shallow projection:

```ts
const wl = useAppFeaturePrefs('watchlist', { entries: [] as Entry[] });

// ✅ fires on add, remove, AND edits to an existing entry's fields
watch(
    () => wl.entries,
    () => mirrorToCloudSql(wl.entries),
    { deep: true }
);

// ❌ fires only on add/remove — silently misses in-place edits
//    (rename, category change, reorder) because .length didn't change
watch(
    () => wl.entries.length,
    () => mirrorToCloudSql(wl.entries)
);
...
The `.length` (or any shallow scalar projection) watcher is the footgun: it
looks like it tracks the array but only re-runs when the count changes, so an
edit to an existing row never reaches your side store and the job snapshots
stale data. Reach for `{ deep: true }` whenever the watch target is an object
or array whose _contents_ (not just identity/length) matter.
