...
### Auto-import scope (don't guess this)

Nuxt auto-imports **`components/`**, **`composables/`**, and (server-side)
**`server/utils/`** — reference their exports by name, no import statement.
But **`utils/` is NOT auto-imported** (the template removes it from the
auto-import dirs in `nuxt.config.ts` to avoid false-positive export
warnings). Import from it explicitly:

```ts
// utils/ — explicit import required:
import { searchEntities } from '~/utils/elementalHelpers';

// composables/ + server/utils/ — auto-imported, just call them:
const { refresh } = useElementalSchema(); // composables/
const schema = await getQsSchema(); // server/utils/ (server only)
```
...
See [storage.md](storage.md) for backend selection and server-side usage (Firestore / KV / Neon Postgres), plus [pref.md](pref.md) for the client-side prefs composables.
...
- **`useAppPrefs` / `useGlobalPrefs` / `useAppFeaturePrefs` /
  `useGlobalFeaturePrefs` / `usePrefsStatus()`** -- per-tenant
  Firestore-backed user preferences (BC 2.0; KV on legacy BC 1.0
  tenants). Reactive objects with auto-debounced persistence; no
  paths, no schemas to register. See [pref.md](pref.md).
