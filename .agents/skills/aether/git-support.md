...
**Push directly to main.** A push to `main` is how this project gets
live — `git push origin main` deploys it. Depending on the `hosting:`
field in `broadchurch.yaml`, that's either the **Deploy Aether UI**
GitHub Actions workflow (`hosting: gcp`, BC 2.0 → Cloud Build →
per-tenant GKE) or a Vercel auto-deploy (`hosting: vercel`, legacy
BC 1.0). Either way the push is the trigger; a PR leaves the change on
a branch, so it never deploys.
