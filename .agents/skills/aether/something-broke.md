...
### `npm run dev` dies immediately with `spawn EBADF`

The dev server crashes on startup (often in Nuxt's `ForkPool` warm-up) with
`Error: spawn EBADF`. This is **file-descriptor exhaustion**, not a code bug:
the Nuxt/Vite watcher recursively crawls a huge directory and blows past the
open-FD limit. The usual culprit is a Python virtualenv in `agents/` — e.g.
`agents/.venv/` holds ~18k files once you've installed agent deps for local
dev (see [local-dev-bc2.md](local-dev-bc2.md)).

Fix: make the watcher ignore `agents/` in `nuxt.config.ts` (the template
already ships this; restore it if it's missing):

```typescript
export default defineNuxtConfig({
    ignore: ['agents/**'],
    vite: { server: { watch: { ignored: ['**/agents/**'] } } },
});
```

This only excludes `agents/` from the **UI** dev watcher — the agent still
runs fine via its own `adk api_server`. Don't chase a phantom code
regression here; the EBADF is environmental (too many watched files).
