...
# Docker build context for the job image (ENG-995 / gap 3).
#   job_dir   — `gcloud builds submit jobs/<name>/` (default). The job is a
#               self-contained program; its Dockerfile only sees the job dir.
#   repo_root — `gcloud builds submit .` with the job's Dockerfile, so a job
#               can `COPY` repo-level shared code (e.g. a `pipeline-strait/`
#               library) that lives outside `jobs/<name>/`. Requires a
#               hand-written Dockerfile in the job dir (the generated default
#               assumes a job-dir context) and respects `.gcloudignore`.
VALID_BUILD_CONTEXTS = ("job_dir", "repo_root")
...
def validate(raw: Any, default_name: str) -> tuple[dict[str, Any], ValidationErrors]:
    ...
    # build_context (ENG-995 / gap 3) — selects the Docker build context.
    # `repo_root` lets a job COPY repo-level shared code; it needs a
    # hand-written Dockerfile (the workflow's generated default assumes a
    # job-dir context), which deploy-job.yml enforces at build time.
    build_context = raw.get("build_context", DEFAULTS["build_context"])
    _check_enum(errs, "build_context", build_context, VALID_BUILD_CONTEXTS)
    ...
    normalized = {
        "name": name,
        "runner": runner,
        "build_context": build_context if isinstance(build_context, str) else "job_dir",
